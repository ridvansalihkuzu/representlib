...
class PixelCL(nn.Module):
    def __init__(
        ...
        DEFAULT_AUG = nn.Sequential(
            #RandomApply(augs.ColorJitter(0.8, 0.2), p=0.8),
            #augs.RandomGrayscale(p=0.2),
            #augs.RandomCrop((image_size,image_size),p=1),
            augs.RandomRotation(degrees=45,p=0.2),
            augs.RandomAutoContrast(p=0.2),
            augs.RandomBrightness(p=0.2),
            augs.RandomChannelShuffle(p=0.2),
            RandomApply(filters.GaussianBlur2d((3, 3), (1.5, 1.5)), p=0.1),
            augs.RandomSolarize(p=0.5),
            augs.Normalize(mean=torch.tensor(0.406), std=torch.tensor(0.225))
        )
        ...
        self.forward(torch.randn(2, channel_size, image_size, image_size, device=device))

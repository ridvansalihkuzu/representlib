...
from represent.models.uc1_pixel_level_contrastive_learning import PixelCL
...
def main(args):
    ...
    def objective(trial):
        ...
        use_pixpro = trial.suggest_categorical('use_pixpro', args.use_pixpro)
        distance_threshold = trial.suggest_categorical('distance_threshold', args.distance_threshold)



        output_file_template = '{}/S{}_L{}_P{}_T{}_D{}_P{}_N{}'.format(args.log_dir, args.input_type,learning_rate, penalty_rate,temperature,distance_threshold,use_pixpro,trial.number)
        ...
        model = PixelCL(
            ...
            distance_thres=distance_threshold,
            # ideal value is 0.7, as indicated in the paper, which makes the assumption of each feature map's pixel diagonal distance to be 1 (still unclear)
            similarity_temperature=temperature,  # temperature for the cosine similarity for the pixel contrastive loss
            alpha=1.,  # weight of the pixel propagation loss (pixpro) vs pixel CL loss
            use_pixpro=True if use_pixpro==1 else False,
        ...
        best_model, best_loss = train_validate(model, data_loader.get_data_loader('train'), data_loader.get_data_loader('valid'), learning_rate,penalty_rate, output_file_template, device)
...
def train_validate(model, train_loader, valid_loader, learning_rate,penalty_rate, output_template, device):
    '''
    ...
    for epoch in range(args.total_epoch):
        ...
        model=train_epoch(model, train_loader, optimizer, lr_scheduler, step, device)
        model.eval()
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument('--distance-threshold', type=float, nargs='+', default=[0.5, 0.7, 0.9])
    parser.add_argument('--use-pixpro', type=int, nargs='+', default=[1,0])

...
model_s2_windstorm="singularity exec --bind "${PWD}:/mnt" --nv  ../represent.sif python -m represent.experiments.uc1_forest_change_map.main_windstorm --input-type 2 --output-layers 1"
...
for folder_num in "${folder_numbers[@]}"; do
    ...
    for pth_file in "${pth_files[@]}"; do
        if [[ $pth_file == *"S2"* ]]; then
            ...
            windstorm_out_dir="represent/result/windstorm/contrastive/$parent_dir/$epoch/"
            echo "Starting Clearcut command: $model_s2_windstorm --out-dir $windstorm_out_dir --model-s2-dir $folder_path/$pth_file"
            $model_s2_windstorm --out-dir $windstorm_out_dir --model-s2-dir $folder_path/$pth_file &
